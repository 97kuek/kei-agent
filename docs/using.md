...
| `#40_knowledge` | 朝の読みもの（技術記事）と、記事や論文の質問 | 知識エージェント |
...
- 毎朝の論文の新着は、知識エージェントがキーワードと前提を見て選ぶ。研究ホームの「先行研究」DB に「未読」で入り、そのスレッドで聞くと知識エージェントが答える。論文は手元（`papers/`）には置かない
...
締切は朝の予定と、24時間前の個別の知らせでも届く。締切まで3日を切っても Notion の状態が「未着手」の課題は、それより先に一度だけ知らせる。Moodle の「0:00」締切は、前の日の「24:00」として表示する。課題の提出の代行はしない。

## 知識（`#40_knowledge`）

- 毎朝7時に、興味のある技術記事を5件、要約と「なぜあなた向けか」つきで1記事ずつ出す（新着がなければ出さない）
- 気になった記事に 👍 を付けると、共通ホームの「読みもの」に「気になる」で入り、Kei Agent が 📝 を付ける。読んだら Notion で状態を「読んだ」にする。👍 を外すと、その行はゴミ箱に入る
- 👍 した記事は、次の日からの選び方の参考になる（出どころや興味が近い記事を少し優先する。興味ごとに偏らせない決まりはそのまま）
- 記事のスレッドで「これ詳しく」、または新しく「これ要約して <URL>」と書くと、記事を読んで答える
- 興味と情報源は、共通ホームの「収集」ページの箇条書きで変える（興味は `名前: キーワード、キーワード`、情報源は `zenn: トピック、…`・`qiita: タグ、…`・RSS の URL）
- 研究の作業（実験・研究ノート）は頼めない。テーマのチャンネルで研究エージェントに頼む
...
| 時刻 | 届くもの | 場所 |
|---|---|---|
| 07:00 | 先行研究の新着（関係のある論文だけ最大5本。新着がなければ投稿しない） | テーマのチャンネル |
| 07:00 | 読みもの（興味のある技術記事5件。1記事ずつ） | `#40_knowledge` |
| 08:00 | 今日の予定（授業・会議・締切）を時刻順に1通。前回の Daily からうまくいかなかった定期処理があれば1行添える。そのスレッドに Daily（今日のタスク／夜間処理の結果／確認待ち・期日・止まっているテーマ・返事待ち／今日考えるとよい問い） | `#01_overview` |
| 21:00 | Retro & Planning（今日の成果／未完了タスク）と「夜間に実行したいタスクはありますか？」。スレッドに明日・明後日の締切 | `#01_overview` |
...
- Moodle の課題は、Daily と Retro & Planning の前に授業ホームの「課題」へ取り込む。新しい課題と締切の変わった課題は `#20_course` に知らせ、Retro & Planning のスレッドには明日・明後日の締切を並べる
- 共通ホームの予定カレンダーには、これからの課題の締切と、朝に読んだ7日ぶんの会議が入る。見つからなくなった会議は消さずに「要確認」になる
- 共通ホームの「今週のタスク」には、締切が今週・来週の研究 Task と課題、期限切れで終わっていないものが、締切の近い順に並ぶ
...
Slack で Kei Agent を開き、[ホーム] タブで次を見たり変えたりできる。見出しと操作だけの1画面。

- 動いているもの … 動いている依頼・ジョブ・返事待ち
- AI … 担当ごとに Claude か Codex
- 定期実行 … チェックでオン・オフ、時刻は欄で変える（先行研究の新着、読みもの、Daily、レトプラ、夜間、保守）
- 声 … 「知らせる」「聞く（マイク）」（どちらも既定は切）
- 接続先 … テーマごとに許可した接続先（足す・外す）
